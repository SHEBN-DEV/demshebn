const DocumentUpload = ({ label, inputId }) => (
    <div className="bg-[#110F0F] flex flex-col items-center justify-center gap-2">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" className="fill-[#ff29d7] size-40">
      ...
      <input type="file" id={inputId} className="hidden" accept=".pdf,.doc,.docx"
        onChange={(e) => handleFileUpload(e.target.files[0])} />
      <label
        htmlFor={inputId}
        className="w-1/2 py-1 bg-black border border-white rounded-3xl text-xs font-medium text-white hover:bg-[#ff29d7] hover:border-[#ff29d7] transition-colors cursor-pointer text-center"
      >
        UPDATE
      </label>
    </div>
